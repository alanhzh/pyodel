...
def sandglass():
    ...
    if form.process(formname="plugin_pyodel_sandglass_form").accepted:
        ...
        if form.vars.done or timedout:
            ...
            # retrieve the updated sandglass
            sandglass = db.plugin_pyodel_sandglass[data["sandglass"]]
        else:
...
def gradebook():
    ...
    grades = db(db.plugin_pyodel_grade.gradebook == gradebook_id).select()

    # TODO:
    # check permissions show/edit record

    # count courses
    courses = set([row.course for row in grades])
    courses.add(None)
...
def grade():
    ...
    if mode == "create":
        ...
        form.vars.gradebook = gradebook_id
    elif mode == "update":
        grade = db.plugin_pyodel_grade[int(request.args[5])]
        form = crud.update(db.plugin_pyodel_grade, grade.id,
...
def desk():
    ...
    attendances = db((db.plugin_pyodel_attendance.student == auth.user_id) & \
    ...
    return dict(attendances=attendances, workspace=workspace,
